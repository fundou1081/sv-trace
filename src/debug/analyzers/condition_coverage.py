...
class ConditionCoverageAnalyzer:
    ...
    def export_to_coverage_model(self, filename: str, report: CoverageReport):
        ...
        import datetime
        lines = []
        lines.append(f"// Condition Coverage Model - Auto-generated by SV-Trace")
        lines.append(f"// Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append("")

        for cov in report.conditions:
            lines.append(f"// ============================================================================")
            lines.append(f"// If at line {cov.line}, depth={cov.depth}")
            lines.append(f"// ============================================================================")
            lines.append("")
            ...
            for i, branch in enumerate(cov.branches):
                ...
                if branch.condition_expr:
                    lines.append(f"// Branch {i}: {branch.condition_expr}")
                
                lines.append(f"covergroup cg_{cov.if_id}_{branch_name} @(posedge clk);")
                lines.append(f"    option.per_instance = 1;")
                lines.append('    option.comment = "Line %d, %s";' % (cov.line, branch.branch_type))
                lines.append("")
                    
                for j, cond in enumerate(branch.expanded_conditions):
                    for sig in cond.expanded_signals[:3]:
                        lines.append(f"    {sig}: coverpoint {sig} {{}};")
                    
                    if len(cond.expanded_signals) >= 2:
                        sigs = ', '.join(cond.expanded_signals[:2])
                        lines.append(f"    cross_{j}: cross {{ {sigs} }};")
                    
                lines.append("endgroup")
                lines.append("")
                lines.append(f"    cg_{cov.if_id}_{branch_name} {cov.if_id}_{branch_name}_inst;")
    ...
    def export_to_coverage_model(self, filename: str, report: CoverageReport):
        ...
        import datetime
        result = []
        result.append("// Condition Coverage Model - Auto-generated by SV-Trace")
        result.append("// Generated: " + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        result.append("")

        for cov in report.conditions:
            result.append("// ============================================================================")
            result.append("// If at line %d, depth=%d" % (cov.line, cov.depth))
            result.append("// ============================================================================")
            result.append("")
            ...
            for i, branch in enumerate(cov.branches):
                ...
                if branch.condition_expr:
                    result.append("// Branch %d: %s" % (i, branch.condition_expr))
                
                result.append("covergroup cg_%s_%s @(posedge clk);" % (cov.if_id, branch_name))
                result.append("    option.per_instance = 1;")
                result.append("    option.comment = \"Line %d, %s\";" % (cov.line, branch.branch_type))
                result.append("")
                    
                for j, cond in enumerate(branch.expanded_conditions):
                    for sig in cond.expanded_signals[:3]:
                        result.append("    %s: coverpoint %s {};" % (sig, sig))
                    
                    if len(cond.expanded_signals) >= 2:
                        sigs = ', '.join(cond.expanded_signals[:2])
                        result.append("    cross_%d: cross { %s };" % (j, sigs))
                    
                result.append("endgroup")
                result.append("")
                result.append("    cg_%s_%s %s_%s_inst;" % (cov.if_id, branch_name, cov.if_id, branch_name))
