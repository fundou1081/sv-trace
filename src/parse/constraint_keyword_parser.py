"""
Constraint Keyword Parser - 使用正确的 AST 遍历
    ...
    def _extract_from_tree(self, root):
        def collect(node):
            ...
            if kind_name in ['ConstraintKeyword', 'PropertyKeyword', 'SequenceKeyword',
                           'EndConstraintKeyword', 'EndPropertyKeyword', 'EndSequenceKeyword']:
...
def extract_constraint_keywords(code: str) -> List[Dict]:
...
constraint c1 { x > 5; }
'''
    result = extract_constraint_keywords(test_code)
    print(f"Constraint keywords: {len(result)}")
