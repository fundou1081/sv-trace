"""
Port Direction Keyword V2 Parser - 使用正确的 AST 遍历

提取 input/output/inout 端口方向关键字：
- InputKeyword
- OutputKeyword
- InoutKeyword
...
class PortDirectionKeywordExtractor:
    def __init__(self):
        self.count: int = 0
...
def extract_port_direction_keywords(code: str) -> List[Dict]:
    return PortDirectionKeywordExtractor().extract_from_text(code)
