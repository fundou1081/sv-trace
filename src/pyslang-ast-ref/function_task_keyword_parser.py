"""
Function Task Keyword Parser - 使用正确的 AST 遍历
...
class FunctionTaskKeywordExtractor:
    def __init__(self):
        self.keywords: List[FunctionTaskKeyword] = []
    ...
    def _extract_from_tree(self, root):
        def collect(node):
            ...
            if kind_name in ['FunctionKeyword', 'EndFunctionKeyword', 'TaskKeyword', 'EndTaskKeyword',
                           'FunctionPort', 'FunctionPrototype']:
                ft = FunctionTaskKeyword()
                ft.keyword = kind_name.replace('Keyword', '').replace('Port', '').lower()
                self.keywords.append(ft)
